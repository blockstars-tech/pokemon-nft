...
contract Fighting is CreatePokemon {
  ...
  function acceptChallenge(uint256 _pokemonReceiverid, uint256 _pokemonChallengerid) public {
    ...
    require(waitingForResponse[_pokemonReceiverid], "not been invited to a challenge!");
    uint256 _number = numbss;
    require(sentAResponse[_pokemonChallengerid], "Pokemon has not sent a Response!");
  ...
  function attackPokemons(uint256 challengedPokemonid, uint256 challengerPokemonid) public {
    ...
    require(sentAResponse[challengerPokemonid], "Enemy pokemon has not sent a response");
  ...
  function _whoIsTheWinner(
    ...
    uint256 percentPokA = (myPokemon.strength * 100 / (myPokemon.strength + enemyPokemon.strength)) - 1;
    // uint winnerId;
    // uint loserId;
    (uint winnerId, uint loserId) = percentPokA < _randomNumber ? (_challengedPokemonid, _challengerPokemonid) : (_challengerPokemonid, _challengedPokemonid);
